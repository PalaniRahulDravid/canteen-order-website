...
function Orders() {
  ...
  const fetchOrders = async () => {
    ...
    if (!user.id) return;
    try {
      const res = await fetch(`${API}/api/orders/user/${user.id}`);
  ...
  // ✅ Show toast when redirected from scanner
  useEffect(() => {
  ...
  const isOrderReceived = (order) => {
    return order.items.every(
      (item) =>
        item &&
        item.item &&
        localStorage.getItem(`received_${order._id}_${item.item._id}`) === 'true'
    );
  };
  ...
  const handleQRScan = (orderId, items) => {
    navigate('/scanner', {
      state: {
        orderId,
        itemIds: items.filter((i) => i.item).map((i) => i.item._id),
      },
    });
  };
                ...
                <div className="order-card card" key={order._id}>
                  ...
                  <div className="order-items">
                    ...
                    <div
                      ...
                      onClick={() =>
                        !received && handleQRScan(order._id, order.items)
                      }
                      style={{
                        cursor: received ? 'default' : 'pointer',
                        opacity: received ? 0.6 : 1,
                      }}
                      title={
                        received ? 'Already received' : 'Scan to receive all'
                      }
                    >
                      <QRCodeSVG
                        value={order._id}
